...
export default function NavAccordionButton({title = "", defaultOpen = false, disabled = false, children, className = "", openFull = false}: NavAccordionProps){
    ...
    return (
        <NavAccordionButtonContext.Provider value={open}>
            <button className={`flex flex-nowrap items-center gap-1 border border-transparent p-1 min-h-7 text-nowrap bg-[var(--theme-background)] active:border-[var(--theme-accent)] ${disabled ? "pointer-events-none" : ""} overflow-hidden ${className}`} onClick={() => setOpen(!open)} disabled={disabled}>
                {open 
                ? <ChevronDownIcon  size={18} className="min-w-[18px] text-[var(--theme-muted)]" />
    
                : <ChevronRightIcon size={18} className="min-w-[18px] text-[var(--theme-muted)]" />
                }
